...
async def post_json(url: str, json_data: dict[str, Any], request_id: str) -> dict[str, Any]:
  async with httpx.AsyncClient(timeout=None) as client:
    headers = {'Authorization': f'Bearer {os.environ.get("OPENAI_API_KEY")}', 'X-Request-Id': request_id}
...
async def handle_request(request: Request):
  ...
  # finish prefill and capture kv params for nixl
  ...
  if kv_transfer_params is not None:
    original_request_data['kv_transfer_params'] = kv_transfer_params
  if kv_transfer_params is None and kv_transfer_params_list is None:
    fallback_params = prefill_request.get('kv_transfer_params')
    if fallback_params:
      original_request_data.setdefault('kv_transfer_params', fallback_params)
    logger.warning('Prefill response missing kv_transfer_params for request %s', request_id)
